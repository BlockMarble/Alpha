...
contract Game /* is usingOraclize */ {
    ...
    */
    ...
    event GameEvent(address indexed addr,uint indexed eventType);
    /*
       	startGame 7 8 
    	joinGame 11 12
    	action 13 14
    ...
    */
    ...
    function join_i(address sender) public{
        if(players.length<playermax && cashes[sender] == 0){
            ...
            players.push(sender);
            emit GameEvent(sender,11);
        }else{
            emit GameEvent(sender,12);
        }
    }
    ...
    function action_i(uint[] params, address sender) public{
        ...
        while(i<params.length){
            ...
            if(actionid>=1&&actionid<=4){
                ...
                if(!move(sender,actionid,val)){
                    emit GameEvent(sender,1123);
                }
            }else if(actionid <= 7 && actionid >= 5){
                
                if(dices[sender] != 0){
                    emit GameEvent(sender,1049);
                    return;
                }
                ...
                if(actionid == 5){ // bid
                    ...
                    landid = uint16(params[i]);
                    i++;
                    uint8 numToBuild = uint8(params[i]);
                    if(landid != wall){
                        built[landid] +=numToBuild;    
                    }
                }
            }else{
                emit GameEvent(sender,125);
            }
        }
        ...
        if(dices[sender] != 0){
            emit GameEvent(sender,7462);
        }
        
        emit GameEvent(sender,1245);
